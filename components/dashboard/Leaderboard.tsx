  ...
  if (!leaderboard) {
    return (
      <div className="rounded-[32px] bg-white border border-slate-100 shadow-[0_25px_80px_rgba(0,0,0,0.08)] p-16 text-center">
        <Trophy size={70} className="mx-auto text-blue-600" />

        <h2 className="mt-6 text-3xl font-bold">Leaderboard Not Available</h2>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="rounded-[32px] bg-white border border-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-8">
          <p className="text-slate-500">Your Current Rank</p>
      ...
      <div className="rounded-[32px] bg-white border border-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-8">
        <h2 className="text-3xl font-bold mb-8">🏆 Top Students</h2>
      ...
      <div className="rounded-[32px] bg-white border border-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-8">
        <h2 className="text-3xl font-bold mb-8">Students Near You</h2>

        <div className="space-y-5">
          {leaderboard.nearbyStudents.map((student) => (
            <div
              key={student._id}
              className="flex justify-between items-center rounded-3xl border border-slate-100 px-6 py-5 hover:shadow-lg transition"
            >
              <div>
                <h3 className="font-bold">{student.student.name}</h3>

                <p className="text-slate-500">Nearby Rank</p>
              </div>

              <div className="flex items-center gap-3">
                <TrendingUp className="text-blue-600" />

                <span className="text-xl font-bold">{student.score}%</span>
              </div>
            </div>
          ))}
        </div>
